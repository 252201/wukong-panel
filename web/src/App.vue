...
<template>
  ...
  <div v-else class="app-shell">
    ...
    <section class="workspace">
      <header class="topbar">
        ...
        <div class="top-actions"><span class="clock">{{ new Date().toLocaleDateString(language, { month: 'short', day: 'numeric' }) }}</span><button class="secondary" @click="openImport">⌁ {{ t('import') }}</button><button class="primary" @click="modal = 'create'">＋ {{ t('deploy') }}</button><span class="avatar">{{ username.slice(0, 1).toUpperCase() }}</span></div>
      </header>
  ...
  <div v-if="modal" class="modal-backdrop" @click.self="modal = null">
    <form v-if="modal === 'create'" class="modal-card node-form" @submit.prevent="createNode"><button type="button" class="modal-close" @click="modal = null">×</button><p class="eyebrow">DEPLOY HY2 NODE</p><h2>部署一座新节点</h2><div class="form-grid"><label class="span-2">节点名称<input v-model="createForm.name" placeholder="例如：花果山 · iPhone" required></label><label>出站策略<select v-model="createForm.mode"><option value="prefer_v6">IPv6 优先 + IPv4 兜底</option><option value="v4only">纯 IPv4</option><option value="v6only">纯 IPv6</option></select></label><label>UDP 端口<input v-model.number="createForm.listenPort" type="number" min="0" max="65535" placeholder="0 = 自动"></label><label>公网域名 / IP<input v-model="createForm.server" placeholder="node.example.com"></label><label>TLS 域名<input v-model="createForm.domain" placeholder="node.example.com"></label><label>IPv4 出站绑定<input v-model="createForm.ipv4Bind" placeholder="自动或 192.0.2.10"></label><label>IPv6 出站绑定<input v-model="createForm.ipv6Bind" placeholder="2001:db8::10"></label><label class="span-2">强制 IPv6 域名<input v-model="createForm.v6OnlyDomains"></label></div><label class="toggle-row inline"><span><b>自动跟随 IP 变化</b><small>动态地址改变时校验配置并重启受影响节点</small></span><span class="switch"><input v-model="createForm.autoBind" type="checkbox"><i></i></span></label><div class="modal-actions"><button type="button" class="secondary" @click="modal = null">取消</button><button class="primary" :disabled="busy">{{ busy ? '正在创建任务…' : '确认部署' }}</button></div></form>
